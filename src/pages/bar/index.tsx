...
import campData from '@sushiswap/sushi-data'
import { t } from '@lingui/macro'
import { tryParseAmount } from '../../functions/parse'
import useActiveWeb3React from '../../hooks/useActiveWeb3React'
import { useLingui } from '@lingui/react'
import useSWR from 'swr'
import useCampBar from '../../hooks/useCampBar'
import { useCampPrice } from '../../services/graph'
...
export default function Stake() {
  ...
  const campBalance = useTokenBalance(account ?? undefined, CAMP[ChainId.MAINNET])
  const xCampBalance = useTokenBalance(account ?? undefined, XCAMP)

  const campPrice = useCampPrice()

  const { enter, leave } = useCampBar()

  const { data } = useSWR(`{bar(id: "0x8798249c2e607446efb7ad49ec89dd1865ff4272") {ratio, totalSupply}}`, fetcher)

  const xCampPerCamp = parseFloat(data?.bar?.ratio)
  ...
  const balance = activeTab === 0 ? campBalance : xCampBalance
  ...
  useEffect(() => {
    const fetchData = async () => {
      const results = await campData.exchange.dayData()
      const apr = (((results[1].volumeUSD * 0.05) / data?.bar?.totalSupply) * 365) / (data?.bar?.ratio * campPrice)

      setApr(apr)
    }
    fetchData()
  }, [data?.bar?.ratio, data?.bar?.totalSupply, campPrice])

  return (
    <Container id="bar-page" className="py-4 md:py-8 lg:py-12" maxWidth="full">
      <Head>
        <title key="title">Stake | Camp</title>
        <meta
          key="description"
          name="description"
          content="Stake CAMP in return for xCAMP, an interest bearing and fungible ERC20 token designed to share revenue generated by all CAMP products."
        />
        <meta key="twitter:url" name="twitter:url" content="https://app.camp.com/stake" />
        <meta key="twitter:title" name="twitter:title" content="STAKE CAMP" />
        <meta
          key="twitter:description"
          name="twitter:description"
          content="Stake CAMP in return for xCAMP, an interest bearing and fungible ERC20 token designed to share revenue generated by all CAMP products."
        />
        <meta key="twitter:image" name="twitter:image" content="https://app.camp.com/xcamp-sign.png" />
        <meta key="og:title" property="og:title" content="STAKE CAMP" />
        <meta key="og:url" property="og:url" content="https://app.camp.com/stake" />
        <meta key="og:image" property="og:image" content="https://app.camp.com/xcamp-sign.png" />
        <meta
          key="og:description"
          ...
          <div className="flex flex-col w-full max-w-xl mt-auto mb-2">
            <div className="flex max-w-lg">
              <div className="self-end mb-3 text-lg font-bold md:text-2xl text-high-emphesis md:mb-7">
                {i18n._(t`Maximize yield by staking CAMP for xCAMP`)}
              </div>
              {/* <div className="self-start pl-6 pr-3 mb-1 min-w-max md:hidden">
                                <img src={XCampSignSmall} alt="xcamp sign" />
                            </div> */}
            </div>
            <div className="max-w-lg pr-3 mb-2 text-sm leading-5 text-gray-500 md:text-base md:mb-4 md:pr-0">
              {i18n._(t`For every swap on the exchange on every chain, 0.05% of the swap fees are distributed as CAMP
                                proportional to your share of the CampBar. When your CAMP is staked into the CampBar, you recieve
                                xCAMP in return for voting rights and a fully composable token that can interact with other protocols.
                                Your xCAMP is continuously compounding, when you unstake you will receive all the originally deposited
                                CAMP and any additional from fees.`)}
            </div>
            {/* <div className="flex">
                            ...
                            <div>
                                <StyledLink className="text-sm text-lg whitespace-nowrap md:text-lg md:leading-5">
                                    Tips for using xCAMP
                                </StyledLink>
                            </div>
                        </div> */}
          </div>
          <div className="hidden px-8 ml-6 md:block w-72">
            <Image src="/xcamp-sign.png" alt="xCAMP sign" width="100%" height="100%" layout="responsive" />
          </div>
        </div>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between w-full h-24 max-w-xl p-4 rounded md:pl-5 md:pr-7 bg-light-yellow bg-opacity-40">
                <div className="flex flex-col">
                  ...
                  <div className="flex">
                    <a
                      href={`https://analytics.camp.com/bar`}
                      target="_blank"
              ...
              <div className="w-full max-w-xl px-3 pt-2 pb-6 rounded bg-dark-900 md:pb-9 md:pt-4 md:px-8">
                ...
                <div className="flex items-center justify-between w-full mt-6">
                  <p className="font-bold text-large md:text-2xl text-high-emphesis">
                    {activeTab === 0 ? i18n._(t`Stake CAMP`) : i18n._(t`Unstake`)}
                  </p>
                  <div className="border-gradient-r-pink-red-light-brown-dark-pink-red border-transparent border-solid border rounded-3xl px-4 md:px-3.5 py-1.5 md:py-0.5 text-high-emphesis text-xs font-medium md:text-base md:font-normal">
                    {`1 xCAMP = ${xCampPerCamp.toFixed(4)} CAMP`}
                  </div>
                </div>
                ...
                <div className="relative w-full h-0 pointer-events-none bottom-14">
                  ...
                  >
                    <div className="flex space-x-2 ">
                      ...
                      >
                        {`${input ? input : '0'} ${activeTab === 0 ? '' : 'x'}CAMP`}
                      </p>
                    </div>
              ...
              <div className="flex flex-wrap">
                <div className="flex flex-col flex-grow md:mb-14">
                  ...
                  <div className="flex items-center space-x-4">
                    <Image
                      ...
                      src="/images/tokens/xcamp-square.jpg"
                      alt="xCAMP"
                      width={64}
                      height={64}
                    />
                    <div className="flex flex-col justify-center">
                      <p className="text-sm font-bold md:text-lg text-high-emphesis">
                        {xCampBalance ? xCampBalance.toSignificant(4) : '-'}
                      </p>
                      <p className="text-sm md:text-base text-primary">xCAMP</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col flex-grow">
                  ...
                  <div className="flex items-center ml-8 space-x-4 md:ml-0">
                    <Image
                      className="max-w-10 md:max-w-16 -ml-1 mr-1 md:mr-2 -mb-1.5 rounded"
                      src="/images/tokens/camp-square.jpg"
                      alt="CAMP"
                      width={64}
                      height={64}
                    />
                    <div className="flex flex-col justify-center">
                      <p className="text-sm font-bold md:text-lg text-high-emphesis">
                        {campBalance ? campBalance.toSignificant(4) : '-'}
                      </p>
                      <p className="text-sm md:text-base text-primary">CAMP</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col w-full mb-4 mt-7 md:mb-0">
                  ...
                  {account && (
                    <a
                      href={`https://analytics.camp.com/users/${account}`}
                      target="_blank"
                      rel="noreferrer noopener"
                      className={`
